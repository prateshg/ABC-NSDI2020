...
if [ $1 = "abc_tcp" ] ; then
	...
	mm-delay 50 mm-link --uplink-log=logs/$2-$1.log --uplink-queue=cellular --uplink-queue-args=\"packets=250,qdelay_ref=50,beta=75\" mahimahi/traces/$2 mahimahi/traces/bw12.mahi ./start-tcp.sh abc
	killall iperf
	killall iperf
fi
...
if [ $1 = "abccubic_tcp" ] ; then
	...
	mm-delay 50 mm-link --uplink-log=logs/$2-$1.log --uplink-queue=cellular --uplink-queue-args=\"packets=250,qdelay_ref=50,beta=75\" mahimahi/traces/$2 mahimahi/traces/bw12.mahi ./start-tcp.sh abccubic
	killall iperf
	killall iperf
fi

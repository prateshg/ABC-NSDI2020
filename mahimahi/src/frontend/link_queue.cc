...
void LinkQueue::record_departure( const uint64_t departure_time, const QueuedPacket & packet )
{
    ...
    if ( log_ ) {
        *log_ << departure_time << " - " << packet.contents.size()
              << " " << departure_time - packet.arrival_time << " " << src << " " << dst << endl;
    }
        ...
        while ( bytes_left_in_this_delivery > 0 ) {
            if ( not packet_in_transit_bytes_left_ ) {
                if ( packet_queue_->empty() ) {
                    if (packet_queue_->to_string().substr(0,3)=="cel" || packet_queue_->to_string().substr(0,3)=="xcp" || packet_queue_->to_string().substr(0,3)=="wei" || packet_queue_->to_string().substr(0,3)=="rcp" || packet_queue_->to_string().substr(0,3)=="vcp") {

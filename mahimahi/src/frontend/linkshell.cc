...
void usage_error( const string & program_name )
{
    ...
    cerr << endl;
    cerr << "          QUEUE_TYPE = infinite | droptail | drophead | codel | pie | cellular | weighted | rcp | vcp | xcp" << endl;
...
unique_ptr<AbstractPacketQueue> get_packet_queue( const string & type, const string & args, const string & program_name )
{
    if ( type == "infinite" ) {
        ...
        return unique_ptr<AbstractPacketQueue>( new CELLULARPacketQueue( args ) );
    } else if ( type == "weighted" ) {
        return unique_ptr<AbstractPacketQueue>( new WEIGHTEDPacketQueue( args ) );
    } else if ( type == "rcp" ) {
        return unique_ptr<AbstractPacketQueue>( new RCPPacketQueue( args ) );
    } else if ( type == "vcp" ) {
        return unique_ptr<AbstractPacketQueue>( new VCPPacketQueue( args ) );
    } else if ( type == "xcp" ) {
        return unique_ptr<AbstractPacketQueue>( new XCPPacketQueue( args ) );
    } else {
